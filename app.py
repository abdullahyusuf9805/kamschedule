...
def load_and_parse_data():
    try:
        ...
        st.error("Data file 'data.xlsx' not found. Please ensure it is in the same folder.")
        st.stop()
    ...
    return df, pd.DataFrame(parsed_rows)

raw_df, parsed_df = load_and_parse_data()

# ==========================================
# 3. DYNAMIC SIDEBAR FILTERS (SUN - THU)
# ==========================================
...
for day_num, (label, default_val) in days_config.items():
    ...
    time_range = st.sidebar.slider(f"{label} Hours", 8, 18, (8, 18)) if is_on else (0, 0)
...
for subj in all_subjects:
    ...
    with st.sidebar.expander(f"📚 {subj_name} ({subj})"):
        teachers_for_subj = sorted(raw_df[raw_df['CODE'].astype(str) == subj]['TEACHER'].astype(str).unique())
...
for subj, rules in subject_rules.items():
    if rules['ban']:
        valid_blocks_df = valid_blocks_df[~((valid_blocks_df['CODE'].astype(str) == subj) & (valid_blocks_df['TEACHER'].isin(rules['ban'])))]
    if rules['require']:
        valid_blocks_df = valid_blocks_df[~((valid_blocks_df['CODE'].astype(str) == subj) & (~valid_blocks_df['TEACHER'].isin(rules['require'])))]

# ==========================================
# 5. DATA GROUPING & SOLVER
# ==========================================
sections_by_subject = {}
for code, group in valid_blocks_df.groupby('CODE'):
    sections_by_subject[str(code)] = []
    for sec_id, sec_group in group.groupby('ID'):
        blocks = [{'day': r['day'], 'start_time': r['start_time']} for _, r in sec_group.iterrows()]
...
if len(target_subjects) < 11:
    st.error(f"Only {len(target_subjects)} valid subjects remaining. (11 required). Check your filters or rules.")
    st.stop()
...
def generate_schedules(subjects_dict, targets):
    ...
    def backtrack(idx, current_schedule, occupied_slots):
        if len(valid_schedules) >= 50: return 
        if idx == len(targets):
        ...
        for section in subjects_dict[targets[idx]]:
            ...
            for b in section['blocks']:
                if (b['day'], b['start_time']) in occupied_slots: overlap = True; break
            if not overlap:
                current_schedule.append(section)
                for b in section['blocks']: occupied_slots.add((b['day'], b['start_time']))
                backtrack(idx + 1, current_schedule, occupied_slots)
                current_schedule.pop()
                for b in section['blocks']: occupied_slots.remove((b['day'], b['start_time']))
    backtrack(0, [], set())
    return valid_schedules

schedules = generate_schedules(sections_by_subject, target_subjects)

# --- RANKING SCHEDULES BY LEAST GAPS ---
...
# ==========================================
# 6. IMAGE GENERATOR & UI RENDERING
# ==========================================
def fix_arabic(text):
...
def draw_schedule_image(schedule):
    ...
    table = ax.table(cellText=cell_text, colLabels=cols_reshaped, loc='center', cellLoc='center')
    table.scale(1, 2)
...
if not schedules:
    ...
    with c_sel:
        selected_idx = st.selectbox(
            ...
            format_func=lambda x: f"Option #{x + 1} (Best Fit)" if x == 0 else f"Option #{x + 1}",
    ...
    # --- VIEW MODE SELECTION TABS (Black = Selected, Dark Gray = Unselected) ---
            ...
            for section in active_sched:
                for b in section['blocks']:
                    if b['start_time'] == hour:
                        ...
                        if c_idx:
                            row_cells[c_idx-1] = f"<b>{section['code']}</b><br><small>(ش {section['id']})</small>"
    ...
    if st.button("Render & Download All Schedules as JPGs (ZIP)", key="download_zip_btn"):
